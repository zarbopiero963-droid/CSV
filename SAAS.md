...
| Fatto | **Backup del database, pezzo idempotenza persistente** (#56): chiude i bloccanti che Sol ha ripetuto a 3a e 3b. **Anti-duplicato (B3):** il percorso cron prenota un **periodo** (data UTC) nella tabella `backup_inviato` (PK) sotto `BEGIN IMMEDIATE` **prima** di inviare — due repliche o un retry non mandano due copie (la seconda `INSERT` è un no-op idempotente `(True, None)`); su invio fallito la prenotazione si **libera**, così un retry della stessa notte riparte. Il **bottone admin** (`periodo` None) è esente e invia sempre. **Ordinamento (B1):** l'high-water-mark `canale_backup_ultimo_update_id` fa uscire come `out_of_order` un `my_chat_member` con `update_id` ≤ all'ultimo processato per il canale (promozione tardiva dopo una rimozione più nuova → niente candidato-zombie). Baratto dichiarato: un crash fra prenotazione e invio salta al più UNA notte (at-most-once sul cron), preferito a due backup. Schema additivo e idempotente (`tests/relay/test_schema.py`). Test fail-first misurati rossi in `tests/relay/test_invio_backup.py` (dedup, liberazione su fallimento, bottone esente) e `tests/relay/test_canale_backup.py` (ordinamento). |
