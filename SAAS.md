...
| `POST /api/admin/canale-backup/conferma` | promuove il candidato a configurato, **solo dopo** un messaggio di prova riuscito | **`404`**; `400` col motivo VISIBILE se la prova fallisce o manca il candidato |
...
| Fatto | **Backup del database, pezzo 2a — backend del canale** (#56): tabella `impostazioni` (chiave→valore, config GLOBALE del proprietario, **non** in `chats`), cattura del canale privato dal webhook **solo se l'azione è dell'amministratore** (`my_chat_member` con status administrator, oppure `forward_from_chat` inoltrato) → scrive un CANDIDATO, e rotte admin (**404** fuori dall'admin): `GET /api/admin/canale-backup` (stato configurato+candidato), `POST …/conferma` (promuove il candidato **solo dopo** un messaggio di prova riuscito, errore visibile se fallisce), `POST …/prova`, `DELETE …/canale-backup` — configurare/rimuovere tracciato in `admin_audit`. La consegna automatica al canale è il pezzo **3**. Test in `tests/relay/test_canale_backup.py` |
