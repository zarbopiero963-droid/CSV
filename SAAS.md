...
| `POST /api/admin/backup/invia` | manda il backup al canale configurato via `sendDocument` (#56 pezzo 3). Due autenticazioni: sessione admin **oppure** header `X-Backup-Cron-Token == BACKUP_CRON_TOKEN` (per il cron notturno). Riverifica la privacy con `getChat` prima di inviare; copia su file e streaming, non in RAM. **Idempotenza (#56):** il percorso **cron** prenota un periodo (data UTC) in `backup_inviato` prima di inviare — retry/repliche non duplicano; su fallimento la prenotazione si libera. Il **bottone admin** (sessione) è esente e invia sempre | **`404`** senza sessione né token valido (token vuoto non autorizza — fail-closed); `400` col motivo se non c'è canale, se è diventato pubblico, o se l'invio fallisce; il cron per un periodo già inviato riceve **`200`** `{inviato:true}` (no-op idempotente) |
...
| Fatto | **Backup del database, pezzo idempotenza persistente** (#56): chiude i bloccanti che Sol ha ripetuto a 3a e 3b. **Anti-duplicato (B3):** il percorso cron prenota un **periodo** (data UTC) nella tabella `backup_inviato` (PK) sotto `BEGIN IMMEDIATE` **prima** di inviare — due repliche o un retry non mandano due copie (la seconda `INSERT` è un no-op idempotente `(True, None)`); su invio fallito la prenotazione si **libera**, così un retry della stessa notte riparte. Il **bottone admin** (`periodo` None) è esente e invia sempre. **Ordinamento (B1):** l'high-water-mark **per canale** `canale_backup_ultimo_update_id:<chat_id>` fa uscire come `out_of_order` un `my_chat_member` con `update_id` ≤ all'ultimo processato per lo **stesso** canale (promozione tardiva dopo una rimozione più nuova → niente candidato-zombie). Per-canale, non globale: l'ordine di un canale non sopprime gli eventi di un altro (bloccante di Fable al gate finale — un secondo candidato non deve bloccare la pulizia del configurato). Baratto dichiarato: un crash fra prenotazione e invio salta al più UNA notte (at-most-once sul cron), preferito a due backup. Schema additivo e idempotente (`tests/relay/test_schema.py`). Test fail-first misurati rossi in `tests/relay/test_invio_backup.py` (dedup, liberazione su fallimento, bottone esente) e `tests/relay/test_canale_backup.py` (ordinamento). |
