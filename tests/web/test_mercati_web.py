"""Avvia il relay e pilota la libreria mercati Betfair (#33) in un browser vero.

Lo script accanto (`mercati_flow.py`) fa il lavoro; qui si tira su il servizio
con la porta a password, come `test_prototype_flow.py` — l'avvio passa da
...
def base_url(tmp_path_factory):
    ...
    with relay_avviato(tmp_path_factory.mktemp('mercati-web'), **ambiente) as base:
        yield base + '/app/'
...
def test_libreria_mercati_e_wizard_a_due_passi(base_url, tmp_path):
    ...
    if proc.returncode != 0:
        raise AssertionError(
            f'mercati_flow.py fallito (exit {proc.returncode})\n'
            f'--- stdout ---\n{proc.stdout[-4000:]}\n'
            f'--- stderr ---\n{proc.stderr[-4000:]}'
        )
